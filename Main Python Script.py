...
game_active = False   #check whether the game is active or not
...
def collision(player, obstacles):
    ...
    if obstacles:     #if there are any obstacles
        for obstacle_rect in obstacles:      #check each obstacle one by one
            if player.colliderect(obstacle_rect):    #if the player touches an obstacle
                game_over_sound.play()
                if score > high_score:
                    high_score = score
                return False   #that means a crash happened, and the game stops
    return True   #if there’s no crash, the game keeps going
